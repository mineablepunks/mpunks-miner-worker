...
__device__ uint64_t device_difficulty;
...
void printHex(const char *title, uint8_t *msg, int len)
{
	if (title)
	{
		printf("%s:\n0x", title);
...
__global__ void Keccak1600(const int inputByte, uint8_t *output, const int outputByte, uint64_t startNounce)
{

	uint32_t num_keccak_blocks = inputByte / (DATA_BLOCK_SIZE << 1);
	// uint8_t *output = device_output[cur];
	// uint32_t *input = device_input[cur];
	...
	uint64_t nonce = startNounce + thread * num_keccak_blocks;
	printf("hi! thread=%llu nounce=%llu num_keccak_blocks=%d blockIdx.x=%d threadIdx.x=%d blockDim.x=%d\n",
		   thread, nonce, num_keccak_blocks, blockIdx.x, threadIdx.x, blockDim.x);
...
uint8_t msg[] = {0x04, 0x22, 0x00, 0x00, 0x00, 0x00, 0x3B, 0x00, 0x19, 0x00, 0x00, 0x00,
...
static mpz_t difficultyTarget_mpz;
static mpz_t nonce_mpz;
...
static const char *DIFFICULTY = "5731203885580";
static const uint64_t difficulty = 5731203885580;
static const char *LASTMINED = "1279043517152342538444603392";
...
// 		if (0)
// 		{
// 			printHex("post nonce", host_input + i * BLOCKSIZE, 32);
// 		}
// 	}
...
void checkOutput()
{
	...
	for (int d = 0; d < BLOCKX * BLOCKNUM; d++)
	{
		...
		printHex("input", input_address, HASH_SIZE);
		printHex("block", output_address, HASH_SIZE);
...
void init()
{
	...
	/* uint88_t */
	// getRandomNumber(host_nonce.arr);
	// printHex("starting nonce", host_nonce.arr, 11);
	// mpz_export(host_difficulty.arr, &count, 1, 11, 1, 0, difficultyTarget_mpz);

	/* set msg */
	printHex("pre msg", msg, 32);

	mpz_export(msg, &count, 1, 12, 1, 0, lastMinedPunkAsset_mpz);
	mpz_export(msg + 12, &count, 1, 9, 1, 0, sender_mpz);
	printHex("pos msg", msg, 32);
	...
	for (int i = 0; i < BLOCKX * BLOCKNUM; i++)
	{
		...
		// printHex("msg",host_input + i*BLOCKSIZE, 32);
...
int main()
{
	...
	// while (1)
	{
		...
		Keccak1600<<<BLOCKNUM, BLOCKX, 0, stream[cur]>>>(BLOCKSIZE / 4, device_output[cur], HASH_SIZE, nonce);
		cudaDeviceSynchronize();
	...
	// {
	// 	printf("block %d:\n", d);
	// 	printHex(NULL, output[d], HASH_SIZE);
	// 	printHex("src", host_input + d * BLOCKSIZE, 32);
	// }
