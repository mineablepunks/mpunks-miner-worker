...
#define CUDA_CALL_OR_RET_X(call, ret)                             \
    do                                                            \
    {                                                             \
        cudaError_t err = call;                                   \
        if (cudaSuccess != err)                                   \
...
__device__ __forceinline__
    uint64_t
    xor1(uint64_t a, uint64_t b)
{
    uint64_t result;
    asm("xor.b64 %0, %1, %2; // xor1"
        : "=l"(result)
        : "l"(a), "l"(b));
    return result;
}
...
__device__ __forceinline__
    uint64_t
    xor3(uint64_t a, uint64_t b, uint64_t c)
{
    uint64_t result;
...
__device__ __forceinline__
    uint64_t
    xor8(uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e, uint64_t f, uint64_t g, uint64_t h)
{
    uint64_t result;
    asm("xor.b64 %0, %1, %2;"
        : "=l"(result)
...
__device__ __forceinline__
    uint64_t
    xandx(uint64_t a, uint64_t b, uint64_t c)
{
#ifdef __CUDA_ARCH__
    uint64_t result;
    asm("{ // xandx \n\t"
...
__device__ __forceinline__
    uint64_t
    andor(uint64_t a, uint64_t b, uint64_t c)
{
#ifdef __CUDA_ARCH__
    uint64_t result;
    asm("{ // andor\n\t"
...
__device__ __forceinline__
    uint64_t
    ROTR64(const uint64_t value, const int offset)
{
    uint2 result;
    if (offset < 32)
    {
...
__device__ __forceinline__
    uint64_t
    ROTR64(const uint64_t x, const int offset)
{
    uint64_t result;
    asm("{ // ROTR64 \n\t"
        ".reg .b64 lhs;\n\t"
...
__device__ __forceinline__
    uint64_t
    ROTL64(const uint64_t value, const int offset)
{
    uint2 result;
    if (offset >= 32)
    {
...
__device__ __forceinline__
    uint64_t
    ROTL64(const uint64_t x, const int offset)
{
    uint64_t result;
    asm("{ // ROTL64 \n\t"
        ".reg .b64 lhs;\n\t"
...
__device__ __forceinline__ void LOHI(uint32_t &lo, uint32_t &hi, uint64_t x)
{
#ifdef __CUDA_ARCH__
    asm("mov.b64 {%0,%1},%2; \n\t"
        : "=r"(lo), "=r"(hi)
...
__device__ __forceinline__
    uint2
    ROR2(const uint2 a, const int offset)
{
    uint2 result;
...
__device__ __forceinline__
    uint2
    ROL2(const uint2 a, const int offset)
{
    uint2 result;
#if __CUDA_ARCH__ > 300
    if (offset >= 32)
...
__device__ __forceinline__ static uint2 SHL2(uint2 a, int offset)
{
#if __CUDA_ARCH__ > 300
    uint2 result;
    if (offset < 32)
...
__device__ __forceinline__ static uint2 SHR2(uint2 a, int offset)
{
#if __CUDA_ARCH__ > 300
    uint2 result;
    if (offset < 32)
