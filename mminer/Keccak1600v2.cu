/*
	...
	author    = {Ling Song and Guohong Liao and Jian Guo},   title     = {{Non-full Sbox Linearization: Applications to Collision Attacks on Round-Reduced Keccak}},
...
texture<unsigned int,1,cudaReadModeElementType> texreference_goal;

#define GET_GOAL(j) (((uint64_t)tex1Dfetch(texreference_goal, 2*j) | (uint64_t)tex1Dfetch(texreference_goal,2*j+1)<<32))
...
#pragma unroll 4
	...
	if(state00 == GET_GOAL(0) && state01 == GET_GOAL(1) && state02 == GET_GOAL(2) && state03 == GET_GOAL(3) && state04 == GET_GOAL(4) &&
			state10 == GET_GOAL(5) && state11 == GET_GOAL(6) && state12 == GET_GOAL(7) && state13 == GET_GOAL(8) && state14 == GET_GOAL(9) &&
			state20 == GET_GOAL(10) && state21 == GET_GOAL(11) && state22 == GET_GOAL(12) && state23 == GET_GOAL(13) && state24 == GET_GOAL(14) &&
			state30 == GET_GOAL(15) && state31 == GET_GOAL(16) && state32 == GET_GOAL(17) && state33 == GET_GOAL(18) && state34 == GET_GOAL(19) &&
			state40 == GET_GOAL(20) && state41 == GET_GOAL(21) && state42 == GET_GOAL(22) && state43 == GET_GOAL(23) && state44 == GET_GOAL(24)){
		//squeezing phase;
#if 0 < HASH_SIZE
	...
	printf("here - %02x\n", output+(blockIdx.x*BLOCKX + threadIdx.x)*HASH_SIZE+32);
...
uint64_t *device_goal[STREAMNUM];
uint64_t goal[25];
...
void GetCudaMalloc(){
	for(int i=0; i<STREAMNUM; i++){
		...
		cudaMalloc(&device_goal[i], sizeof(uint64_t)*25);
		checkCUDAError("malloc for device_goal");
	}
...
int main(){
	...
	for(int i=0; i<TESTROUND; i++){
		...
		cudaMemcpyAsync(device_goal[cur], goal, sizeof(uint64_t)*25, cudaMemcpyHostToDevice, stream[cur]);
		cudaBindTexture(0,texreference_input, device_input[cur], SUMDATASIZE);
		cudaBindTexture(0,texreference_goal, device_goal[cur], sizeof(uint64_t)*25);
		...
		cudaMemcpyAsync(output, device_output[cur], HASH_SIZE*BLOCKNUM*BLOCKX, cudaMemcpyDeviceToHost, stream[cur]);
		cur = (cur + 1) % STREAMNUM;
		cudaUnbindTexture(&texreference_goal);
